"""
class to train and test encoder and decoder models
"""
...
class TrainTestEncoderDecoder:
    def __init__(self, data, encoder, decoder, learning_rate=0.01,
    ...
    def train_iteration(self, X, y_history, y_target, use_cuda=True):
    ...
    def predict(self, on_train=False, use_cuda=True):
        ...
        while i < len(y_pred):
            ...
            for j in range(len(batch_idx)):
                if on_train:
                    ...
                    X[j, :, :] = self.X[range(batch_idx[j] + self.train_size - self.T, batch_idx[j] + self.train_size),
                                 :]
                    y_history[j, :] = self.y[
                        range(batch_idx[j] + self.train_size - self.T, batch_idx[j] + self.train_size)].flatten()
            ...
            if use_cuda:
                ...
                # yht (N, T, 1)
                yht = yht.unsqueeze(2)
                features = self.encoder(Xt).cuda()
                pred_cuda = self.decoder(features, yht).cuda()
